...
# zone_stat_dict = {}
# ZoningArray = zones_arr
# MapArray = ndvi_arr
# LabelsDict = labels_dict
# CsvOutputName = 'Temp_ZonalStat.csv'
# for zone in np.arange(1,11):
#     index_arr = np.where(ZoningArray == zone , 1, np.nan)
#     arr_ = np.where ( index_arr == 1, MapArray, np.nan)
#     arr_2 = MapArray[~np.isnan(arr_)]
#     zone_stat_dict [labels_dict[zone]]  = [np.round(arr_2.min(),3),
#                                             np.round(arr_2.max(),3),
#                                             np.round(arr_2.mean(),3),
#                                             np.round(np.abs(arr_2).mean(),3),
#                                             np.round(arr_2.var(),3),
...
ZonalStatRastertoRaster(temp_arr, ndvi_arr, 'Temp_ZonalStat.csv', labels_dict)    
    ...
    # temp_arr_2 = temp_arr_[temp_arr_ != np.nan]
    
    
    
                           
    # print(ndvi_arr_2.size, temp_arr_2.size)
    
    

    # plt.imshow(index_arr)
    # plt.show()

# index_arr = np.where(zones_arr == 8 , 1, np.nan)
# print(np.unique(index_arr))
# plt.imshow(index_arr)
# plt.show()
